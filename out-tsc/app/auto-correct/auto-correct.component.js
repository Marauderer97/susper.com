    ...
    function AutoCorrectComponent(autocorrectservice, route, activatedroute, store, ref) {
        ...
        this.query$.subscribe(function (query) {
            if (query) {
                _this.sugflag = false;
                _this.isQues = false;
                _this.isQues = query.substr(query.length - 1) === '?';
                _this.autocorrectservice.getsearchresults(query.replace(/[?!]/g, "")).subscribe(function (res) {
                    _this.sugflag = false;
                    if (res) {
                        if (res['original'].replace(/[.,?!]/g, "").toLocaleLowerCase() !== res['suggestion'].replace(/[.,?!]/g, "").toLocaleLowerCase()) {
                            _this.sugflag = true;
                            _this.suggestion = _this.isQues === true ? res['suggestion'] + '?' : res['suggestion'];
                        }
                        else {
                            _this.sugflag = false;
                        }
